...
/* ═══════════════════════════════════════════════════════════════════
   BORDER GREY THREADS
   Neutral greyscale palette for colour-agnostic border previews.
   Mirrors LAYER_SLOT_GREYS — distinguishable without implying
   actual thread colours. Actual colours come from the shoutout.
═══════════════════════════════════════════════════════════════════ */
const BORDER_GREY_THREADS = [
  {id:1, name:'Slot 1', dmc:'', hex:'#111111'},
  {id:2, name:'Slot 2', dmc:'', hex:'#404040'},
  {id:3, name:'Slot 3', dmc:'', hex:'#707070'},
  {id:4, name:'Slot 4', dmc:'', hex:'#999999'},
  {id:5, name:'Slot 5', dmc:'', hex:'#C0C0C0'},
  {id:6, name:'Slot 6', dmc:'', hex:'#E4E4E4'},
];
  ...
  useEffect(function() {
    ...
    for (let r = 0; r < CROP; r++) {
      for (let c = 0; c < CROP; c++) {
        const kind = grid[r][c];
        if (kind === ' ') continue;
        const col = stitchColor(kind, BORDER_GREY_THREADS);
      ...
      <div className="sheet">
        ...
        <div className="detail-section">
          ...
          <CrossStitchCanvas word="ABC" cols={94} rows={94}
            borderStyle={border.spec || border.style} threads={BORDER_GREY_THREADS} size={400}/>
          <div className="form-hint" style={{marginTop:6}}>
            Shown in neutral greys — actual colours depend on the shoutout this border is used in.
          </div>
        </div>
...
const COMPOSE_POSITIONS = [
  {id:'topLeft',    label:'Top left',    isCorner:true,  maxW:42, maxH:42},
  {id:'top',        label:'Top',         isCorner:false, maxW:42, maxH:42},
  {id:'topRight',   label:'Top right',   isCorner:true,  maxW:42, maxH:42},
  {id:'left',       label:'Left',        isCorner:false, maxW:42, maxH:42},
  {id:'centre',     label:'',            isCorner:false, maxW:0,  maxH:0},
  {id:'right',      label:'Right',       isCorner:false, maxW:42, maxH:42},
  {id:'bottomLeft', label:'Bottom left', isCorner:true,  maxW:42, maxH:42},
  {id:'bottom',     label:'Bottom',      isCorner:false, maxW:42, maxH:42},
  {id:'bottomRight',label:'Bottom right',isCorner:true,  maxW:42, maxH:42},
];
    ...
    <div className="obj-picker-overlay open"
      ...
      <div className="obj-picker-sheet">
        ...
        <div className="obj-picker-hint">
          Tap an object to place it. Any overlap with borders or text will show in the canvas.
        </div>
        <div className="obj-list">
          ...
          {objects.map(function(obj) {
            ...
            return (
              <div key={obj.id}
                className="obj-card"
                onClick={function() { onSelect(normObj); }}>
                ...
                <div className="obj-card-slots">
                  {pw}×{ph} stitches · {layerCount === 1 ? '1 colour' : layerCount + ' colours'}
                </div>
              </div>
            );
          })}
